...
.code`
    ...
    source = source.replace(/^class([\s\S]+?)(?<num>\:[0-9]+)\{([\s\S]+?)(\k<num>)\}/gm,match=>{
        ...
        match=match.replace(/constructor\(\)(?<num>\:[0-9]+)\{([\s\S]+?)(\k<num>)\}/gm,mmm=>{
            ...
            funcs['constructor'] = {data:makeReturn(bdy),params:[]}
            return ''
        })
        match=match.replace(/[a-zA-Z0-9]+\(.*(?<num>\:[0-9]+)\{([\s\S]+?)(\k<num>)\}/gm,mmm=>{
            ...
            funcs[fname] = {data:makeReturn(data),params}
            return ''
        })
        ...
        for(const func of Object.keys(funcs)){
            ...
            var index = 0
            try{
            data = data.replace(/this\.([a-zA-Z0-9\_]+)\(/gm,name+'_$1(qword ptr self,')
        }catch(e){}
            console.log(data)
            functions.push('function '+name+'_'+func+'(this:'+name+params+'){\n'+data+'\n}\n')
        }
 ...
 //       `
    ...
    for(const key of Object.keys(OBJECTS)){
        ...
        for(const param of OBJ.params){
            ...
            source = source.replace(new RegExp(param+'\\.[a-zA-Z0-9\_]+\\(','gm'),match=>{
                ...
                return `${key}_${func}(qword ptr ${obj},`
            })
        }
    }
    ...
    source = source.replace(/([a-zA-Z0-9\_]+\.[a-zA-Z0-9\_]+) = (.*)/gm,'mov $1, $2')
    ...
    replaceFunctionZ(/function\ Macro([a-zA-Z0-9\_]+)\(\)(?<num>\:[0-9]+)\{([\s\S]+?)(\k<num>)\}/gm)
    replaceFunctionZ(/function\ Macro([a-zA-Z0-9\_]+)\(([\s\S]+?)\)(?<num>\:[0-9]+)\{([\s\S]+?)(\k<num>)\}/gm, true)
    replaceFunctionZ(/function\ Macro([a-zA-Z0-9\_]+)\(\)\{([\s\S]+?)\}/gm)
    replaceFunctionZ(/function\ Macro([a-zA-Z0-9\_]+)\(([\s\S]+?)\)\{([\s\S]+?)\}/gm, true)
