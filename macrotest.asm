; ¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤

    include \masm64\include64\masm64rt.inc

    include \masm64\include64\opengl32.inc
    include \masm64\include64\glu32.inc

    include \danger\include\opengl.inc
    include \danger\include\requires.inc

    includelib \masm64\lib64\opengl32.lib
    includelib \masm64\lib64\glu32.lib

    include \danger\include\extern.inc

    .data

        text1 db "test tekst",0
        indexWhile dq 10
        numB dq 0
        numC dq 2

    .code

    ; Przykład rekurencyjnego makra
myRecursiveMacro MACRO numA
    LOCAL recursivePart
    
    invoke printf , "OK"

    cmp numA, 0

    jz recursivePart ; Jeśli N = 0, zakończ rekurencję

    invoke printf , "OK22"

    ; Wywołanie rekurencyjne z N-1
    ;dec numA
    ;myRecursiveMacro numA

recursivePart:
ENDM

; ¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤¤



entry_point proc


    myRecursiveMacro numB

    myRecursiveMacro numC


    invoke ExitProcess,0

    ret

entry_point endp


    end
